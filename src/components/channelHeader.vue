<template>
	<div>
		<header
			...
			class="channelBanner"
			:style="{
				background: 'url(' + dataArray[0].authorBanners[1].url + ')',
			}"
		>
			<div v-if="myWidth > 900" class="bannerContent">
				<figure
					class="avatar avatar-xxl"
					:data-initial="dataArray[0].author.charAt(0)"
				>
					<img
						:src="dataArray[0].authorThumbnails[2].url"
						:alt="dataArray[0].author"
					/>
				</figure>
				<div>
					<h1 class="channelTitle">{{ dataArray[0].author }}</h1>
					<h5 class="channelSubtitle">
						Subscribers: {{ dataArray[0].formattedSubs }}
					</h5>
				</div>
			</div>
		</header>
