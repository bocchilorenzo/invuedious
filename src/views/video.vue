			...
			<div v-if="failed">
				<div class="empty">
					...
					<p class="empty-subtitle">
						The request to the indvidious instance took too long, or
	...
	methods: {
			...
			axios({
				...
				.then((response) => {
					...
					this.videoInfo[0].thumb =
						this.videoInfo[0].videoThumbnails[0].url
					for (
						let i = 0;
						i < this.videoInfo[0].formatStreams.length;
						i++
					) {
		...
		async downloadCaptions() {
			...
			for (let i = 0; i < this.videoInfo[0].captions.length; i++) {
				await axios(
					this.$store.state.selected +
						this.videoInfo[0].captions[i].url
				)
					.then((res) => {
						this.videoInfo[0].downloadedCaptions.push({
							...
							languageCode:
								this.videoInfo[0].captions[i].languageCode,
						})
					})
					.catch((err) => {
						console.log(err)
